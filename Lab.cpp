...
int main() {
  ...
  zoo.push_back(std::make_unique<Wolf>(Wolf(2)));
  zoo.push_back(std::make_unique<Chicken>(Chicken(42)));
  zoo.push_back(std::make_unique<Kangaroo>(std::make_unique<Kangaroo>(std::make_unique<Chicken>(8))));
  int chicken_count = 0;
  for (auto &a : zoo) {
    a->sound();
  ...
  std::cout << chicken_count << std::endl;
  return 0;
}
